...
var DEFAULT_GMAIL_CONTACT_TO = "chowdorydevops@gmail.com";
function getRuntimeEmailEnvironment() {
  return {
...
function getEmailConfiguration(env = getRuntimeEmailEnvironment()) {
  return {
    demoMode: env.GMAIL_DEMO_MODE !== "false",
    contactTo: env.GMAIL_CONTACT_TO || env.GMAIL_SMTP_USER || DEFAULT_GMAIL_CONTACT_TO
  };
}
function getTransport(env = getRuntimeEmailEnvironment()) {
...
var appRouter = router({
  ...
  contact: router({
    send: publicProcedure.input(z2.object({
      ...
      project: z2.string().trim().min(10).max(5e3)
    })).mutation(async ({ input }) => {
...
async function handler(req, res) {
  if (!app) {
    const created = express2();
    created.use(express2.json({ limit: "50mb" }));
    created.use(express2.urlencoded({ limit: "50mb", extended: true }));
